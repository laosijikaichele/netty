...
public class HttpStatusValueOfBenchmark extends AbstractMicrobenchmark {
    ...
    private static void fillBenchMarkData(int size, CircularLink head) {
        ...
        do {
            ...
            // 38%
            if (code < 38) {
                cl.value = random.nextInt(100, 110);
                ++c1x;
                continue;
            }
            // 30%
            if (code < 68) {
                cl.value = random.nextInt(200, 215);
                ++c2x;
                continue;
            }
            // 15%
            if (code < 83) {
                cl.value = random.nextInt(300, 315);
                ++c3x;
                continue;
            }
            // 10%
            if (code < 93) {
                cl.value = random.nextInt(400, 435);
                ++c4x;
                continue;
            }
            // 5%
            if (code < 98) {
                cl.value = random.nextInt(500, 515);
                ++c5x;
                continue;
            }
            // 2%
            cl.value = random.nextInt(-25, 25);
            ++c6x;
